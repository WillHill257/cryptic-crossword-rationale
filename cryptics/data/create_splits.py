...
def make_random_split(all_clues: List[Clue], seed=42) -> Tuple[List[Clue]]:
    ...
    logger.info(
        "Random Split: Train Size      = {} of {}".format(len(train), len(all_clues))
    )
    logger.info(
        "Random Split: Validation Size = {} of {}".format(len(val), len(all_clues))
    )
    logger.info(
        "Random Split: Test Size       = {} of {}".format(len(test), len(all_clues))
    )
...
def make_disjoint_split(
    all_clues: List[Clue], word_initial_split: bool, seed=42
) -> Tuple[List[Clue]]:
    ...
    logger.info(
        "{} Disjoint Split: Train Size      = {} of {}".format(
            "Word-Initial" if word_initial_split else "Naive",
            len(train),
            len(all_clues),
        )
    )
    logger.info(
        "{} Disjoint Split: Validation Size = {} of {}".format(
            "Word-Initial" if word_initial_split else "Naive", len(val), len(all_clues)
        )
    )
    logger.info(
        "{} Disjoint Split: Test Size       = {} of {}".format(
            "Word-Initial" if word_initial_split else "Naive", len(test), len(all_clues)
        )
    )
...
def write_splits_to_json(
    dataset_name: str, train: List[Clue], validation: List[Clue], test: List[Clue]
) -> None:
    # build the object representation of all the data, then convert to json
    # loop through the splits, converting each element to an object
    data = {
        "train": {"data": [elem.to_map() for elem in train]},
        "validation": {"data": [elem.to_map() for elem in validation]},
        "test": {"data": [elem.to_map() for elem in test]},
    }

    # create the files, one per split (overwrite if it exists)
    for k, v in data.items():
        filename = f"json/{dataset_name}/{k}.json"
        f = open(filename, "w")
